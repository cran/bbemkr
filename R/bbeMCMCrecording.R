bbeMCMCrecording <-
function(data_x, data_y, x, costpara, num_batch = 50, M = 10000, step = 20, sizep = 1.2)
{
   dim = length(x)
   size_batch = M/num_batch
   length2 = M/step
   sum_h = total_sd = xp = h = sif = std_h = vector(, dim + 3)
   ...
   for(k in 1:M)
   {
       dummy = np_gibbs(data_x, data_y, xh, inicost, sizep = sizep)
   xh = dummy$xh
   sigma = dummy$sigma
   ...
   xp[dim+2] = plike[k] = bbelike(data_x, data_y, xh, sigma)
...
total_sd[i] = sqrt(abs(M*(total_sd[i]/M-sum_h[i]*sum_h[i])/(M-1)))
h[i] = sum_h[i]
}
for(i in 1:(dim+3))
{
...
logmarginal = log(1/temp) + sum_h[dim+2]
    logmargin = bbeloglikelihood(data_x, data_y, sum_h[1:dim], sum_h[dim+1])
...
return(list(accept_raterecording = sum(accept_htest)/M,         
        sum_h = sum_h, std_h = std_h, batch_h = batch_h, total_sd = total_sd, 
        sif = sif, R2 = kern(data_x, sum_h[1:dim]), data_post = data_post, 
        logmarginalNR = logmarginal, loglikelihoods = logmargin, logprior = temp,
logdensity = temp2, logmarginalChib = logmarginal2))
}
