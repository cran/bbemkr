np_gibbs = function (xh, inicost, k, mutsizp, prob, data_x, data_y, prior_p, prior_st) 
{
    ...
    fy = cost(xh, data_x, data_y, prior_p = prior_p, prior_st = prior_st)
    r = fx - fy
	if(class(r) != "numeric")
	{
    ...
    c = mutsizp * ((1 - 1/dm) * sqrt(2 * pi) * exp(alpharm^2/2))/(2 * 
        alpharm) + 1/(dm * prob * (1 - prob))
    if (accept == 1) {
        accept_h = 1
        inicost = fy
    ...
    tmp = cost2(xh, data_x, data_y, prior_st = prior_st)
    un = rgamma(1, 0.5 * (data_num + prior_p), tmp)
    sigma2 = 1/un
    return(list(x = xh, sigma2 = sigma2, cost = inicost, accept_h = accept_h, 
        mutsizp = mutsizp))
}
